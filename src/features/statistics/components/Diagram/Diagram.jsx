...
const CustomTooltip = ({ active, payload }) => {
  ...
  const { name, totalAmount, rainbowColor } = payload[0].payload;
...
const Diagram = ({ expenseStatistics, totalExpense, colorsMap }) => {
  ...
  const renderRainbowSector = props => {
    ...
    return <Sector {...rest} fill={payload?.rainbowColor || fill} />;
  };
      ...
      <ResponsiveContainer width="100%" aspect={1}>
        <PieChart style={{ outline: 'none' }}>
          ...
          <Pie
            ...
            dataKey="totalAmount"
            nameKey="name"
            shape={renderRainbowSector}
          >
            <Label position="center" fill="#000" className="text-lg font-bold">
              {`₴ ${totalExpense
